...
chaff block add evil.example.com # заблокировать вручную
...
`--json` к любой команде даёт JSON для скриптов.

Функции можно включать и выключать на лету, без перезапуска демона
(в TUI — `space` на вкладке «Функции»):
...
Вкладки: Статус · Срабатывания · Функции · Списки · Блокировки. Обновляется
сама; `?` — помощь, `a` — авто-обновление, `/` — поиск по блокировкам. `enter`
на блокировке или срабатывании — заблокировать / разрешить / снять ручное.
