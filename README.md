...
Приоритет решений сверху вниз: **ручное правило → групповое → список-фид**. То есть
текущие ручные («фундаментальные») блокировки всегда перевешивают групповые правила,
а групповое правило, конфликтующее с фундаментальным, создать нельзя. Одна машина не
может состоять больше чем в одной группе (чтобы правила не конфликтовали).

```
chaff module enable grouppolicy
chaff group add инженеры --action block --note "карантин"
chaff group add-member инженеры ноутбук-05      # по имени хоста
chaff group add-member инженеры aa:bb:cc:dd:ee:ff
chaff group enable инженеры                      # политика применяется
chaff group scan                                 # кандидаты из сети (имя|mac)
chaff group ls
chaff group disable инженеры
```

Участники по имени хоста «ждут», пока namesnoop не выучит их MAC из DHCP/mDNS; до
этого политика к ним не применяется. Функционал экспериментальный — интерфейс это
подписывает и в CLI, и в панели.
